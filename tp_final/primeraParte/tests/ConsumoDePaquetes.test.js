...
describe("Compra de paquetes", () => {
    test("Cliente compra un paquete, el sistema le debita de la cuenta", () => {
        ...
        const paquete_basico = new Paquete(datos = 1, minutos = 100, dias = 7, precio = 150)
    ...
    test("Cliente adquiere un paquete, intenta adquirir otro antes de que se agote o termine el paquete actual, el sistema lo bloquea.", () => {
        const cliente = new Cliente(nombre = "Pepe", linea = 1123456789)
        const paquete_basico = new Paquete(datos = 1, minutos = 100, dias = 7, precio = 150)
        const paquete_intermedio = new Paquete(datos = 2, minutos = 500, dias = 7, precio = 300)
...
describe("Consumo de datos.", () => {
    ...
    test("Cliente consume algunos datos del paquete, sabe exactamente cuantos datos , minutos y dias le quedan.", () => {
        ...
        cliente.cargarEnCuenta(450)
        cliente.comprarPaquete(paquete_basico, fecha = new Date("01/01/2024"))

        expect(cliente.consume(new Consumo(datos = 0.1, minutos = 20, fecha = new Date("01/01/2024")))).toEqual("Le quedan: 0.9 GB y 80 minutos. Vence en 7 días.")
        expect(cliente.consume(new Consumo(datos = 0.1, minutos = 20, fecha = new Date("01/02/2024")))).toEqual("Le quedan: 0.8 GB y 60 minutos. Vence en 6 días.")
    })
    ...
    test("Cliente adquiere un paquete, se consumen los datos, adquiere otro paquete.", () => {
        ...
        cliente.cargarEnCuenta(450)
        cliente.comprarPaquete(paquete_basico, fecha = new Date("01/01/2024"))
        cliente.consume(new Consumo(datos = 1, minutos = 0, fecha = new Date("01/08/2024")))

        expect(cliente.comprarPaquete(paquete_intermedio, fecha = new Date("01/08/2024"))).toEqual("Paquete comprado: 2 GB, 500 minutos, 7 dias, 300 pesos.")
...
describe("Historial de consumo.", () => {
    test("Se quiere obtener el historial entero de consumo para un cliente.", () => {
        const cliente = new Cliente(nombre = "Pepe", linea = 1123456789)
        const paquete_basico = new Paquete(datos = 10, minutos = 100, dias = 7, precio = 150)
